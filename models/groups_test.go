...
func testGroupToManyUsers(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"group_members\" (\"group_id\", \"user_id\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"group_members\" (\"group_id\", \"user_id\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
...
var (
	groupDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `ZoomLink`: `character varying`, `Published`: `boolean`, `Archived`: `boolean`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
