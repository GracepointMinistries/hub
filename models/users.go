...
func (userL) LoadZgroups(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("\"zgroups\".id, \"zgroups\".name, \"zgroups\".archived, \"zgroups\".created_at, \"a\".\"user_id\""),
		qm.From("\"zgroups\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Archived, &one.CreatedAt, &localJoinCol)
		if err != nil {
